              ...
              <button disabled={!amount || amount <= 0}
                onClick={async () => {
                  try {
                    ...
                    navigate("/paymentstatus?message=" + encodeURIComponent(errorMessage));
                  }
                }}
                className="justify-center rounded-md text-sm font-medium ring-offset-background transition-colors h-10 px-4 py-2 w-full bg-green-500 text-white disabled:bg-green-300 disabled:cursor-not-allowed"
              >
                Initiate Transfer
              </button>
              <button
